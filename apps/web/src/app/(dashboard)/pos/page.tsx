...
function PaymentModal({ total, onConfirm, onClose, isPending }: {
  total: number;
  onConfirm: (method: string, paidAmount: number, note: string, delivery?: DeliveryData) => void;
  onClose: () => void;
  isPending: boolean;
}) {
  ...
  const [delivery,   setDelivery]   = useState<DeliveryData>({
    address: '', city: '', phone: '', instructions: '', shippingCost: 0,
  });
  const inputRef = useRef<HTMLInputElement>(null);

  useEffect(() => { setTimeout(() => inputRef.current?.focus(), 50); }, []);

  const deliveryTotal = total + (isDelivery ? (delivery.shippingCost || 0) : 0);
  const receivedNum   = parseFloat(received) || 0;
  const change        = method === 'CASH' ? Math.max(0, receivedNum - deliveryTotal) : 0;
  const canConfirm    = (method !== 'CASH' || receivedNum >= deliveryTotal)
                     && (!isDelivery || (delivery.address.trim() && delivery.city.trim()));
      ...
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="text-center py-3 rounded-xl bg-nexus-500/10 border border-nexus-500/20">
            ...
            <p className="text-4xl font-bold text-nexus-500 tabular-nums">{formatCurrency(deliveryTotal)}</p>
            {isDelivery && delivery.shippingCost > 0 && (
              <p className="text-xs text-muted-foreground mt-1">
                Incluye envío {formatCurrency(delivery.shippingCost)}
              </p>
            )}
          </div>
          ...
          <div
            ...
            className={cn(
              'flex items-center gap-3 px-3 py-2.5 rounded-xl border-2 cursor-pointer transition-all select-none',
              isDelivery
          ...
          >
            ...
            <div className="flex-1">
              <p className="text-sm font-medium">¿Es domicilio?</p>
              <p className="text-xs opacity-70">
                {isDelivery ? 'Irá al pipeline de Pedidos Online' : 'Venta en mostrador'}
              </p>
            </div>
            <div className={cn(
              'w-9 h-5 rounded-full transition-colors relative shrink-0',
              isDelivery ? 'bg-emerald-500' : 'bg-muted-foreground/30',
            )}>
              <div className={cn(
                'absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all',
                isDelivery ? 'left-4' : 'left-0.5',
              )} />
            </div>
          </div>
          ...
          {isDelivery && (
            <div className="space-y-2 p-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5">
              ...
              <Input
                placeholder="Indicaciones (apto, color puerta...)"
              ...
              <div className="flex items-center gap-2">
                ...
                <Input
                  ...
                  onChange={e => setDelivery(d => ({ ...d, shippingCost: Number(e.target.value) || 0 }))}
                  className="h-9 text-sm"
                  min={0}
                />
              </div>
          ...
          {method === 'CASH' && (
            <div className="space-y-3">
              <Input
                ...
                placeholder={`Monto recibido (mín. ${formatCurrency(deliveryTotal)})`}
              ...
              {received && !canConfirm && (
                <div className="flex items-center gap-2 p-2.5 rounded-lg bg-rose-500/10 border border-rose-500/20 text-xs text-rose-500">
                  ...
                  Faltan {formatCurrency(deliveryTotal - receivedNum)}
                </div>
              )}
            </div>
          )}
        ...
        <div className="flex gap-3 p-5 pt-0">
          ...
          <Button
            onClick={() => onConfirm(
              method,
              method === 'CASH' ? receivedNum : deliveryTotal,
              note,
          ...
          >
            {isPending
              ...
              : isDelivery
                ? <><Bike className="h-4 w-4" />Confirmar domicilio</>
...
export default function POSPage() {
  ...
  const handleConfirmPayment = (
    ...
    createOrder.mutate({
      ...
      channel:        delivery ? 'PHONE' : 'POS',
      deliveryMethod: delivery ? 'HOME_DELIVERY' : 'PICKUP',
